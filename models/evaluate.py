...
class Sampler:
    ...
    def sample_animation(self, n_frames: int = 1000, create_video: bool = True):
        ...
        for t_inv in monit.iterate('Denoise', self.n_steps):
            ...
            # $t$ in a tensor
            '''
            new_full是PyTorch张量（Tensor）的一个方法，用于根据现有张量的属性创建一个新的填充了相同值的张量。这个方法确保了新张量与原始张量有同样的数据类型（dtype）和位于同一设备上（比如CPU或者GPU）。

            下面是new_full方法的详细解释：
            
            假设我们有一个现有的张量xt，在你给出的例子里，xt是一个随机生成的4维张量，其维度是[1, self.image_channels, self.image_size, self.image_size]。这个张量可能表示一个图像批次，其中只有一张图像，且图像的通道数、高度与宽度由对象的属性定义。
            
            当我们调用xt.new_full(size, fill_value, dtype=None, ...)时，我们请求创建一个新的张量，其维度由size参数指定，所有元素初始化为fill_value。如果指定dtype参数，新张量将具有该数据类型，若不指定，则保持与xt相同。在你的代码里，
            
            t = xt.new_full((1,), t_, dtype=torch.long)
             
            这一行创建了一个新的一维张量t，其中只有一个元素。该元素会被填充为t_中存储的值（假设t_是之前代码中定义的一个初值），且张量的数据类型设置为torch.long。
            
            new_full是一个非常有用的方法，用于初始化具有指定形状、值和数据类型的新张量，并保证这个新张量和原张量在相同的设备上（无论是CPU还是GPU）。这在深度学习中尤其有用，因为它有助于管理设备之间的数据迁移和保持类型一致性。
            '''
            t = xt.new_full((1,), t_, dtype=torch.long)
            # $\textcolor{lightgreen}{\epsilon_\theta}(x_t, t)$

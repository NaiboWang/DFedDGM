...
class FedProx(BaseLearner):
    ...
    def _local_update(self, model, train_data_loader, idx):
        ...
        for iter in range(self.args["local_ep"]):
            for batch_idx, (_, images, labels) in enumerate(train_data_loader):
                ...
                loss = F.cross_entropy(output, labels) + (0.01 / 2) * proximal_term
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        return model.state_dict()
    ...
    def _local_finetune(self, model, train_data_loader, idx):
        ...
        for iter in range(self.args["local_ep"]):
            for batch_idx, (_, images, labels) in enumerate(train_data_loader):
                ...
                # loss = loss_function(y_pred, label)
                loss = F.cross_entropy(output[:, self._known_classes:], fake_targets) + (0.01 / 2) * proximal_term # 只看新的类别的loss和acc，如_known_classes=20，那么只看20-39的类别的loss和acc
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    def _fl_train(self, train_dataset, test_loader):
        ...
        # with open(f'user_groups_{self.args["dataset"]}_task{self.args["tasks"]}_{self.args["num_users"]}_{self._cur_task}.pkl', "wb") as f:

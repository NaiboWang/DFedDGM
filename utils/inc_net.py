...
class IncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes):
        fc = self.generate_fc(self.feature_dim, nb_classes) # nb_classes每次都会增加，比如第一次是20，第二次是40，直到100（对应cifar100）
        if self.fc is not None:
            nb_output = self.fc.out_features
            weight = copy.deepcopy(self.fc.weight.data)
            bias = copy.deepcopy(self.fc.bias.data)
            fc.weight.data[:nb_output] = weight # 比如当前的fc的weight是20*64，新的fc的weight是40*64，那么就是将前20行的weight（即之前训练好的weights)赋值给新的fc的weight，后面的40-20行的weight为默认值
    ...
    def forward(self, x):
        ...
        out = self.fc(x["features"])
        """
        在Python中，update方法通常是字典（dict）的一个方法，用于更新一个字典的键值对。如果你有一个字典A，并且调用A.update(B)，那么字典B中的键值对将会被添加到字典A中，如果有相同的键，则会覆盖A中对应的键的值。
        """
        out.update(x)
        if hasattr(self, "gradcam") and self.gradcam:
